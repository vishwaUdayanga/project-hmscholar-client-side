...
import { AddAnnouncementSchema } from "@/app/lib/zod-schema"
import { z } from "zod"
...
export default function EditAnnouncementForm({announcement_id}: {announcement_id: string}) {
    ...
    useEffect(() => {
        const fetchAnnouncement = async () => {
            try {
                ...
                const sectionsResponse = await getAnnouncement({ announcementId: announcement_id });
                if (!sectionsResponse.ok) {
                    throw new Error('Failed to fetch announcement');
                }
                const announcement: Announcement = await sectionsResponse.json();
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            const response = await updateAnnouncement({ announcement_id: announcement_id, announcement_title: data.title, announcement_description: data.description });
            if (response.ok) {
                setIsLoading(false)
                setButtonText('Save Changes')
                alert('Announcement updated successfully')
                handleClear()
                window.location.href = `/lecturer/dashboard/view-course/${announcement?.course_id}`
            } else {
                setIsLoading(false)
                setButtonText('Save Changes')
